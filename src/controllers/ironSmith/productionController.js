...
// Monthly production counts for Iron Smith based on job_order date_range.from
export const getIronMonthlyProductionCounts = asyncHandler(async (req, res) => {
  try {
    ...
    const result = await ironDailyProduction.aggregate([
      {
        $lookup: {
          from: 'ironjoborders',
      ...
      {
        $match: {
          'job_order_data.date_range.from': { $gte: startOfYear, $lt: endOfYear }
        }
      },
      {
        $group: {
          _id: { month: { $month: "$job_order_data.date_range.from" } },
          count: { $sum: 1 }
        }
      },
    ...
    console.error('Iron monthly counts error:', error);
...
const addQcCheck_25_09_2025 = asyncHandler(async (req, res) => {
  try {
    const { job_order, shape_id, object_id, rejected_quantity, remarks } = req.body;
...
const addQcCheck_30_09_2025 = asyncHandler(async (req, res) => {
  try {
    ...
    // Update quantities in dailyProduction (using the total from qcCheck for consistency)
    ...
    dailyProduction.production_logs.push({
      ...
      user: req.user._id,
      description: remarks || 'N/A',
      rejected_quantity: qcCheck.rejected_quantity, // Use the total rejected_quantity
    });
